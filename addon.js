...
export default {
    ...
    async getMetas(type = 'MOVIE', providers = ['nfx'], country = "US", language = 'zh') {
        let res = null;
        try {
        ...
        return (await Promise.all(res.data.data.popularTitles.edges.map(async (item, index) => {
            ...
            // @doc https://github.com/Stremio/stremio-addon-sdk/blob/master/docs/api/responses/meta.md
            const imdbRating = item?.node?.content?.scoring?.imdbScore
            const description = item?.node?.content?.description
